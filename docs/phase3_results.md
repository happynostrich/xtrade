...
**结论：T1–T7 全 PASS（含 replay-parity T7 fill-by-fill 严格相等）；T8 收尾 PASS，并在编写 schema test 时顺手抓到并修复一个 `_PaperBridge` 中潜伏的 BUY/SELL 反向 bug。**
...
| T6  | testnet 端到端 | PASS（手工 runbook） | `xtrade.live.signal_runner.run_live_signal` 复用 Phase 1 `run_live` testnet 工厂；`xtrade live signal-run` CLI；**6 类错误分级 → 退出码 0/1/2**；`docs/phase3_runbook_testnet.md` 操作员步骤 + 验证 checklist + 失败模式表；`tests/test_live_signal_runner.py`（13）+ `tests/test_cli_live_signal.py`（3，全 offline，注入 `live_executor` 桩）（commit `5edad2c`） |
...
- **`SignalConsumer` cursor 仅维护内存**：进程崩溃后重启会从 queue 头重放；Phase 4 可考虑把 cursor 落盘到 `data/cursors/<strategy>.json`。
...
- T6 走的是手工 runbook 路径（brief §5 Task 6 明确"不写自动化测试"），自动化覆盖到 venue hop 之前；操作员按 `docs/phase3_runbook_testnet.md` §4 的 6 项 checklist 验证后即可签收。
